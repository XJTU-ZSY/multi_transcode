...
void tsenc::readaudio(ULONG pts)
{
	pthread_mutex_lock(&audiolock);
	
	while (audiopts[audio_raddr] < pts)
	{
		ts_write(audiobuf[audio_raddr], audiolen[audio_raddr], 0, audiopts[audio_raddr], 1);
		audio_raddr++;
...
void tsenc::writeinfo(unsigned char* p, unsigned char a, unsigned char b, int size, unsigned char day, epginfo*  epginfo1)
{
	pthread_mutex_lock(&sccd);
	if (a != infostor[2] || b != infostor[3])
	{
		if ((abs(b - infostor[3]) < 4) && a == infostor[2])
		{	
		...
		infostor[0]=day; 
		infostor[1] = size + 2;
		memcpy(&infostor[2], &a, 1);
		memcpy(&infostor[3], &b, 1);
		memcpy(&infostor[4], p - size, size);
		num = size + 4;
	}
	for (int i = 0; i < 13; i++)
	{
		memcpy(&fuinfo[i], &epginfo1[i], sizeof(epginfo));
	}
	pthread_mutex_unlock(&sccd);
}
...
void tsenc::ts_write(PBYTE buf, mfxU32 len, ULONG dts, ULONG pts, int type)
{
				...
				if (type == 0)
				{
					...
					if (bKeyFrame)
					{
						...
						*p++ = 0x50; // random access + PCR
						p = Write_PCR(p, dts - 72000);
							ULONG timestamp = 0;
							timestamp = pts / 90;
							unsigned char* c = new unsigned char[2 + num];
							c[0] = (timestamp >> 8) & 0xFF;
							c[1] = timestamp & 0xFF;
							memcpy(&c[2], infostor, num);
							USHORT CRCCHAR = 0;
							CRCCHAR = usMBCRC16(c, 2 + num);
							memcpy(p, c, 2 + num);
							p += 2 + num;
							memcpy(p, &CRCCHAR, 2);
							p += 2;
